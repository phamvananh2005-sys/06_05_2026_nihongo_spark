...
const dict = {
  vi: {
    ...
    shadowingDesc: "Nghe mẫu, bắt chước lại theo từ vựng hoặc câu mẫu, rồi tự nghe lại bản ghi của mình để luyện đến khi tự nhiên hơn.",
    ...
    listenSlow: "Chậm",
    listenNormal: "Chuẩn",
    yourTurn: "Nghe mẫu, thu âm lại phần bắt chước, sau đó nghe lại bản ghi của mình trước khi chuyển tiếp.",
  ...
  en: {
    ...
    shadowingTitle: "Shadowing",
    shadowingDesc: "Listen to the model, imitate vocabulary or sentences, then replay your own recording for self-practice.",
    ...
    listenSlow: "Slow",
    listenNormal: "Normal",
    yourTurn: "Listen to the model, record your shadowing, then replay your recording before moving on.",
...
function clampScore(val) {
  return Math.min(10.0, Math.max(0.0, parseFloat(val) || 0)).toFixed(1);
}

function getRubricLabels(mode, t) {
  if (mode === 'vocab') {
    return {
      syllable_accuracy: 'Phát âm âm tiết',
      long_vowel: 'Trường âm',
      sokuon: 'Âm ngắt っ',
      hatsuon: 'Âm mũi ん',
  ...
  if (mode === 'sentence') {
    return {
      pronunciation: t('cPronunciation'),
      intonation: 'Ngữ điệu',
      rhythm: 'Rhythm / Nhịp điệu',
      speed: 'Tốc độ',
      similarity: 'Similarity / Độ giống mẫu'
    };
  }
  if (mode === 'topic') {
    return {
  ...
  return {
    ...
    grammar: t('cGrammar'),
    vocabulary: t('cVocab'),
    idea_development: t('cIdeaDev')
  };
}

function getRubricWeights(mode) {
  if (mode === 'vocab') {
    return { syllable_accuracy: 0.50, long_vowel: 0.15, sokuon: 0.10, hatsuon: 0.10, pitch_accent: 0.15 };
  }
  if (mode === 'sentence') {
    return { pronunciation: 0.30, intonation: 0.20, rhythm: 0.15, speed: 0.15, similarity: 0.20 };
  }
  if (mode === 'topic') {
    return { pronunciation: 0.25, grammar: 0.20, vocabulary: 0.15, content: 0.25, fluency: 0.15 };
  }
  return { pronunciation: 0.25, fluency: 0.25, grammar: 0.20, vocabulary: 0.15, idea_development: 0.15 };
}

function calculateWeightedScore(scores, mode) {
  const weights = getRubricWeights(mode);
  let total = 0;
  Object.entries(weights).forEach(([key, weight]) => {
    total += (parseFloat(scores?.[key]) || 0) * weight;
  });
  return clampScore(total);
}

function getPerformanceLevel(score, lang) {
...
function getBandTemplate(score, mode, lang) {
  const n = parseFloat(score) || 0;
  const isEn = lang === 'en';

  if (mode === 'vocab') {
    if (n >= 9.5) return isEn ? '10/10 band: accurate syllables, long vowels, sokuon, ん, and natural pitch accent.' : 'Band 10/10: phát âm chính xác âm tiết, trường âm, âm ngắt, âm ん và pitch accent tự nhiên.';
    if (n >= 8) return isEn ? '8–9 band: mostly accurate; only a few small long-vowel or pitch-accent issues.' : 'Band 8–9: đọc đúng phần lớn; chỉ còn một vài lỗi nhỏ về trường âm hoặc pitch accent.';
    if (n >= 6) return isEn ? '6–7 band: understandable, but sokuon, ん, or vowel clarity needs work.' : 'Band 6–7: người nghe vẫn hiểu được từ, nhưng cần luyện âm ngắt っ, âm ん hoặc nguyên âm rõ hơn.';
    return isEn ? '4–5 band: many syllables are inaccurate; practise slowly with the model first.' : 'Band 4–5: sai nhiều âm tiết; cần nghe mẫu và đọc chậm từng âm trước khi đọc cả từ.';
  }

  if (mode === 'sentence') {
    if (n >= 9.5) return isEn ? '10/10 band: accurate pronunciation, natural rhythm, and Japanese-like intonation.' : 'Band 10/10: phát âm chính xác, nhịp câu tự nhiên, ngữ điệu gần giống người Nhật.';
    if (n >= 8) return isEn ? '8–9 band: clear and well-paced; improve phrase breaks and final intonation.' : 'Band 8–9: đọc rõ ràng, tốc độ phù hợp; cần chỉnh một vài chỗ ngắt cụm hoặc ngữ điệu cuối câu.';
    if (n >= 6) return isEn ? '6–7 band: understandable, but sounds word-by-word and lacks natural rhythm.' : 'Band 6–7: nội dung vẫn dễ hiểu nhưng còn đọc rời từng từ, rhythm chưa tự nhiên.';
    return isEn ? '4–5 band: many words are hard to recognize; practise vocabulary before the full sentence.' : 'Band 4–5: nhiều từ phát âm sai, AI khó nhận diện toàn bộ câu; cần luyện từ riêng trước.';
  }

  if (mode === 'topic') {
    if (n >= 9.5) return isEn ? '10/10 band: complete content, personal details, natural language, clear pronunciation, and fluent delivery.' : 'Band 10/10: đề cập đầy đủ yêu cầu, có thông tin cá nhân, ngôn ngữ tự nhiên, phát âm rõ và nói trôi chảy.';
    if (n >= 8) return isEn ? '8–9 band: covers most requirements and uses some original expression; improve minor grammar and vocabulary variety.' : 'Band 8–9: hoàn thành hầu hết yêu cầu, có cách diễn đạt riêng; cần chỉnh vài lỗi ngữ pháp nhỏ và tăng độ đa dạng từ vựng.';
    if (n >= 6) return isEn ? '6–7 band: answers the topic but misses some required points and has short/simple sentences.' : 'Band 6–7: trả lời đúng chủ đề nhưng còn bỏ sót một số ý bắt buộc, nhiều câu ngắn và còn ngập ngừng.';
    return isEn ? '4–5 band: related to the topic but too short, missing key content, and has basic grammar errors.' : 'Band 4–5: có liên quan đến chủ đề nhưng thiếu nhiều ý quan trọng, sai ngữ pháp cơ bản hoặc bài nói quá ngắn.';
  }

  if (n >= 9.5) return isEn ? '10/10 band: clear ideas, examples, natural language, and strong pronunciation.' : 'Band 10/10: ý tưởng rõ ràng, có ví dụ minh họa, ngôn ngữ tự nhiên và phát âm tốt.';
  if (n >= 8) return isEn ? '8–9 band: has a personal view; develop ideas more deeply and vary vocabulary.' : 'Band 8–9: có quan điểm cá nhân; cần giải thích sâu hơn và đa dạng hóa từ vựng.';
  if (n >= 6) return isEn ? '6–7 band: expresses basic ideas, but pauses often and repeats vocabulary.' : 'Band 6–7: diễn đạt được ý cơ bản nhưng ý còn đơn giản, nhiều khoảng dừng và lặp từ.';
  return isEn ? '4–5 band: too short or unclear; use opinion → reason → example.' : 'Band 4–5: nội dung quá ngắn hoặc lập luận chưa rõ; nên trả lời theo công thức Ý kiến → Lý do → Ví dụ.';
}

function normalizeGradingResult(apiRes, mode, lang, t) {
  const labels = getRubricLabels(mode, t);
  const rawScores = apiRes?.rubric_scores || {};

  // Backward-compatible fallback if the model returns old field names.
  const legacyScores = {
    pronunciation: apiRes?.pronunciation_score,
    fluency: apiRes?.fluency_score,
    grammar: apiRes?.grammar_score,
    vocabulary: apiRes?.vocab_score,
    content: apiRes?.content_score || apiRes?.accuracy_score,
    idea_development: apiRes?.idea_development_score || apiRes?.vocab_score,
    syllable_accuracy: apiRes?.syllable_accuracy || apiRes?.pronunciation_score,
    long_vowel: apiRes?.long_vowel || apiRes?.pronunciation_score,
    sokuon: apiRes?.sokuon || apiRes?.accuracy_score,
    hatsuon: apiRes?.hatsuon || apiRes?.accuracy_score,
    pitch_accent: apiRes?.pitch_accent || apiRes?.fluency_score,
    intonation: apiRes?.intonation || apiRes?.fluency_score,
    rhythm: apiRes?.rhythm || apiRes?.fluency_score,
    speed: apiRes?.speed || apiRes?.fluency_score,
    similarity: apiRes?.similarity || apiRes?.accuracy_score
  };

  const rubricScores = {};
  Object.keys(labels).forEach(key => {
    rubricScores[key] = clampScore(rawScores[key] ?? legacyScores[key] ?? apiRes?.score ?? apiRes?.overall_score ?? 0);
  });

  const weightedScore = calculateWeightedScore(rubricScores, mode);
  const score = clampScore(apiRes?.overall_score ?? apiRes?.score ?? weightedScore);

  const criteria = {};
  Object.entries(labels).forEach(([key, label]) => {
    criteria[label] = rubricScores[key];
  });

  const strengths = Array.isArray(apiRes?.strengths) ? apiRes.strengths : [];
  const mistakes = Array.isArray(apiRes?.mistakes) ? apiRes.mistakes : [];
  const tips = Array.isArray(apiRes?.practice_tips) ? apiRes.practice_tips : [];
  const band = apiRes?.band_feedback || getBandTemplate(score, mode, lang);

  const sectionTitles = lang === 'en'
    ? { band: 'Rubric band', strengths: 'Strengths', mistakes: 'Specific errors to fix', tips: 'Practice suggestions' }
    : { band: 'Mức điểm theo rubric', strengths: 'Điểm mạnh', mistakes: 'Lỗi cần sửa cụ thể', tips: 'Gợi ý luyện tập' };

  const parts = [];
  parts.push(`${sectionTitles.band}: ${band}`);
  if (strengths.length) parts.push(`${sectionTitles.strengths}:\n${strengths.map(x => `✓ ${x}`).join('\n')}`);
  if (mistakes.length) parts.push(`${sectionTitles.mistakes}:\n${mistakes.map(x => `✗ ${x}`).join('\n')}`);
  if (tips.length) parts.push(`${sectionTitles.tips}:\n${tips.map(x => `→ ${x}`).join('\n')}`);
  if (apiRes?.feedback) parts.push(apiRes.feedback);

  return {
    score,
    level: apiRes?.level || getPerformanceLevel(score, lang),
...
function generateGradingResultFallback(transcript, expectedRawText, level, mode, lang, t) {
  ...
  const matchRate = targetString ? Math.min(1.0, matchCount / Math.max(1, cleanExpectedKanji.length || cleanExpectedHiragana.length)) : 0;

  let rubric_scores = {};
  if (mode === 'vocab') {
    const base = matchRate ? matchRate * 10 : Math.min(8.0, 4.0 + cleanTranscript.length / 2);
    rubric_scores = {
      syllable_accuracy: clampScore(base),
      long_vowel: clampScore(base - 0.4),
      sokuon: clampScore(base - 0.5),
      hatsuon: clampScore(base - 0.5),
      pitch_accent: clampScore(base - 0.8)
    };
  } else if (mode === 'sentence') {
    const base = matchRate ? matchRate * 10 : Math.min(8.0, 4.0 + cleanTranscript.length / 8);
    rubric_scores = {
      pronunciation: clampScore(base),
      intonation: clampScore(base - 0.5),
      rhythm: clampScore(base - 0.4),
      speed: clampScore(base - 0.2),
      similarity: clampScore(base - 0.3)
    };
  } else if (mode === 'topic') {
    const base = cleanTranscript.length < 15 ? 4.0 : Math.min(9.0, 5.5 + cleanTranscript.length / 45);
    rubric_scores = {
      pronunciation: clampScore(base - 0.3),
      grammar: clampScore(base - 0.4),
      vocabulary: clampScore(base - 0.2),
      content: clampScore(base),
      fluency: clampScore(base - 0.1)
    };
  } else {
    const base = Math.min(9.0, 4.5 + cleanTranscript.length / 55);
    rubric_scores = {
      pronunciation: clampScore(base - 0.3),
      fluency: clampScore(base - 0.2),
      grammar: clampScore(base - 0.4),
      vocabulary: clampScore(base - 0.2),
      idea_development: clampScore(base)
    };
  }

  const score = calculateWeightedScore(rubric_scores, mode);
  const estimatedLevel = cleanTranscript.length > 120 ? 'N2' : cleanTranscript.length > 70 ? 'N3' : cleanTranscript.length > 25 ? 'N4' : 'N5';

  return normalizeGradingResult({
    overall_score: score,
    level: getPerformanceLevel(score, lang),
    estimated_jlpt: mode === 'free' ? estimatedLevel : '',
    rubric_scores,
    strengths: lang === 'en' ? ['The system could identify parts of the speech.'] : ['Hệ thống nhận diện được một phần bài nói của bạn.'],
    mistakes: lang === 'en' ? ['This fallback evaluation cannot reliably identify every pronunciation detail.'] : ['Đây là chấm dự phòng nên chưa thể bóc tách thật chính xác từng lỗi phát âm.'],
    practice_tips: lang === 'en' ? ['Use direct recording and speak clearly for a more detailed AI evaluation.'] : ['Dùng chế độ thu âm trực tiếp và nói rõ từng cụm để nhận phản hồi chi tiết hơn.']
  }, mode, lang, t);
}

const evaluateWithGemini = async (transcript, expectedText, level, mode, lang, requirement = '') => {
  const apiKey = import.meta.env.VITE_OPENAI_API_KEY;
  const systemPrompt = `You are a strict but encouraging native Japanese speaking examiner.
Feedback language: ${lang === 'en' ? 'English' : 'Vietnamese'}.
Task Mode: ${mode}.
- vocab = Shadowing theo từ
- sentence = Shadowing theo câu
- topic = Nói theo chủ đề có hướng dẫn / Semi-Guided Speaking
- free = Nói tự do
Student JLPT target level: ${level}.
Topic Requirement / required points: "${requirement || 'None'}"
Model Answer / Target Text: "${expectedText || 'None'}"
Student Voice Transcript: "${transcript || ''}"

CORE RULES:
1. Grade ONLY the student's Japanese speaking shown in the transcript.
2. For vocab and sentence modes, compare strictly with the Model Answer / Target Text.
3. For topic mode, the Model Answer is ONLY a suggestion. The student may use their own words, add personal experience, and be creative. Grade mainly by whether they satisfy the Topic Requirement / required points.
4. For free mode, DO NOT judge whether the topic is right/wrong, whether it matches a model, or whether it follows a prompt. Only evaluate real Japanese ability.
5. Always point out specific errors. Do NOT write vague comments like "grammar needs improvement" or "pronunciation needs practice". Mention the exact word, phrase, particle, or structure when possible.
6. Vietnamese tone rule: if feedback language is Vietnamese, always call the student "bạn". Refer to yourself as "Hệ thống" or "AI". Never use "em", "thầy", "cô", "mình", or "tôi".

RUBRICS AND WEIGHTS:
A) vocab / Shadowing từ:
- syllable_accuracy 50%: correct mora/syllables, no missing sounds, no added sounds.
- long_vowel 15%: long vowels such as おばさん / おばあさん.
- sokuon 10%: small っ, e.g. きて / きって.
- hatsuon 10%: nasal ん, e.g. しんぶん.
- pitch_accent 15%: beginner level can be graded lightly.

B) sentence / Shadowing câu:
- pronunciation 30%: each word is pronounced correctly.
- intonation 20%: natural Japanese sentence intonation.
- rhythm 15%: natural phrase grouping, e.g. わたしは｜きのう｜ともだちと｜えいがを｜みにいきました.
- speed 15%: not too slow or too fast.
- similarity 20%: closeness to the target sentence.

C) topic / Nói theo chủ đề có hướng dẫn:
- pronunciation 25%.
- grammar 20%.
- vocabulary 15%.
- content 25%: covers required points.
- fluency 15%.

D) free / Nói tự do:
- pronunciation 25%: clarity, long vowels, sokuon, ん.
- fluency 25%: continuous speech, few pauses, suitable speed.
- grammar 20%: particles, verb conjugation, sentence patterns.
- vocabulary 15%: variety and appropriateness.
- idea_development 15%: connected sentences, explanation, examples.

BAND FEEDBACK GUIDE:
- 9.5-10: excellent, no significant errors.
- 8-9.4: good, mostly complete, only minor errors.
- 6-7.9: understandable/basic completion, but several missing points or fluency/accuracy issues.
- 4-5.9: attempted but too short, many basic errors, or hard to recognize.

Return ONLY valid JSON. No markdown. Use this exact schema:
{
  "overall_score": "0.0 to 10.0 weighted total",
  "level": "Vietnamese: Xuất sắc/Giỏi/Khá/Cần cố gắng OR English: Excellent/Good/Fair/Needs Practice",
  "estimated_jlpt": "N5/N4/N3/N2/N1 for free mode, otherwise empty string",
  "rubric_scores": {
    "syllable_accuracy": "0.0 to 10.0 only for vocab",
    "long_vowel": "0.0 to 10.0 only for vocab",
    "sokuon": "0.0 to 10.0 only for vocab",
    "hatsuon": "0.0 to 10.0 only for vocab",
    "pitch_accent": "0.0 to 10.0 only for vocab",
    "pronunciation": "0.0 to 10.0 for sentence/topic/free",
    "intonation": "0.0 to 10.0 only for sentence",
    "rhythm": "0.0 to 10.0 only for sentence",
    "speed": "0.0 to 10.0 only for sentence",
    "similarity": "0.0 to 10.0 only for sentence",
    "grammar": "0.0 to 10.0 only for topic/free",
    "vocabulary": "0.0 to 10.0 only for topic/free",
    "content": "0.0 to 10.0 only for topic",
    "fluency": "0.0 to 10.0 only for topic/free",
    "idea_development": "0.0 to 10.0 only for free"
  },
  "strengths": ["specific strength 1", "specific strength 2"],
  "mistakes": ["specific error with exact student phrase and correction if possible"],
  "practice_tips": ["concrete practice tip 1", "concrete practice tip 2"],
  "band_feedback": "one concise sentence matching the score band and mode",
  "feedback": "short final encouraging comment"
}`;
  ...
  const payload = {
    ...
    messages: [
      ...
      { role: "user", content: "Grade this student's speech using the selected rubric and return JSON only." }
    ],
    temperature: 0.15,
    max_tokens: 1200
  };
...
export const AudioInput = ({ onAudioReady, disableAI = false }) => {
    ...
    <div className="grid md:grid-cols-2 gap-4">
      ...
      >
        ...
        <p className="text-xs text-slate-500 mt-1">
          {disableAI ? 'Dùng để nghe lại, không chấm AI' : 'Hệ thống sẽ giả lập chấm điểm'}
        </p>
      </div>
      ...
      >
        {!isRecording && (
          <div className="absolute top-0 right-0 bg-[#F26522] text-white text-[10px] font-bold px-2 py-0.5 rounded-bl-lg">
            {disableAI ? 'Khuyên dùng' : 'Khuyên dùng AI'}
          </div>
        )}
        ...
        {isRecording ? (
          ...
          <>
            ...
            >
              {disableAI ? 'Thu âm để nghe lại' : 'Chấm điểm bằng giọng nói'}
            </button>
          </>
        )}
      </div>
  ...
  const startGrading = async () => {
    ...
    try {
      ...
      if (isFileUpload) {
        ...
        const modeForRubric = type === 'topic' ? 'topic' : 'free';
        finalResult = normalizeGradingResult({
          overall_score: clampScore(baseScore),
          level: getPerformanceLevel(baseScore, lang),
          estimated_jlpt: type === 'free' ? 'N4' : '',
          rubric_scores: modeForRubric === 'topic'
            ? {
                pronunciation: clampScore(baseScore - 0.2),
                grammar: clampScore(baseScore - 0.3),
                vocabulary: clampScore(baseScore - 0.1),
                content: clampScore(baseScore),
                fluency: clampScore(baseScore - 0.2)
              }
            : {
                pronunciation: clampScore(baseScore - 0.2),
                fluency: clampScore(baseScore),
                grammar: clampScore(baseScore - 0.3),
                vocabulary: clampScore(baseScore - 0.1),
                idea_development: clampScore(baseScore - 0.2)
              },
          strengths: lang === 'en' ? ['The audio file was received successfully.'] : ['Hệ thống đã nhận được file âm thanh của bạn.'],
          mistakes: lang === 'en' ? ['Uploaded files cannot be checked for detailed pronunciation timing as reliably as direct recording.'] : ['File tải lên không thể bóc tách lỗi phát âm chi tiết ổn định như chế độ thu âm trực tiếp.'],
          practice_tips: lang === 'en' ? ['Use Direct Record for the most accurate rubric-based grading.'] : ['Bạn nên dùng Thu âm trực tiếp để AI chấm chi tiết hơn theo rubric.']
        }, modeForRubric, lang, t);
      } else {
        ...
        if (!transcript || transcript.trim().length === 0) {
          finalResult = normalizeGradingResult({
            ...
            rubric_scores: type === 'topic'
              ? { pronunciation: '2.0', grammar: '2.0', vocabulary: '2.0', content: '2.0', fluency: '2.0' }
              : { pronunciation: '2.0', fluency: '2.0', grammar: '2.0', vocabulary: '2.0', idea_development: '2.0' },
            strengths: lang === 'en' ? ['You attempted the recording.'] : ['Bạn đã thử thực hiện bài nói.'],
            mistakes: lang === 'en' ? ['The system could not clearly recognize the speech.'] : ['Hệ thống không nhận diện rõ bạn nói gì.'],
            practice_tips: lang === 'en' ? ['Check the microphone and speak louder, one phrase at a time.'] : ['Kiểm tra microphone và nói rõ hơn, từng cụm ngắn một.']
          }, type, lang, t);
        } else {
          const apiRes = await evaluateWithGemini(transcript, expectedText, levelTarget, type, lang, topicRequirement);
          if (apiRes) {
            finalResult = normalizeGradingResult(apiRes, type, lang, t);
          } else {
...
function ShadowingMode({ studentName, onRequireName, dbShadowing }) {
  ...
  const handleAudioReady = async (file, url) => {
    // Shadowing chỉ dùng để nghe mẫu → thu âm → nghe lại.
    // Không gọi transcribe, không gọi evaluateWithGemini, không tạo điểm AI.
      ...
      <div className="bg-white/95 backdrop-blur-md rounded-3xl shadow-xl p-6 md:p-8 border border-[#f0e0d8]">
        ...
        {!recordedFile && !isEvaluating && (
          <div className="animate-in fade-in">
            ...
            <AudioInput onAudioReady={handleAudioReady} disableAI={true} />
          </div>
        )}

        {recordedFile && !isEvaluating && (
          <div className="animate-in slide-in-from-bottom-4">
            <div className="p-6 rounded-2xl border shadow-sm bg-green-50 border-green-200 mb-6 flex flex-col md:flex-row gap-6 items-center md:items-start">
              <div className="w-20 h-20 rounded-full flex items-center justify-center shadow-inner shrink-0 bg-green-500 text-white">
                <CheckCircle2 size={36} />
              </div>

              <div className="flex-1 w-full text-center md:text-left">
                <h4 className="font-bold text-slate-800 mb-2 text-lg">
                  {lang === 'en' ? 'Recording saved' : 'Đã lưu bản thu'}
                </h4>
                <p className="text-sm text-slate-700 mb-4 leading-relaxed font-medium">
                  {lang === 'en'
                    ? 'Listen to your recording, compare it with the model, then try again or move to the next item.'
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-8 pt-6 border-t border-slate-200">
              <button onClick={() => { setRecordedFile(null); setRecordedUrl(null); setSentenceResult(null); }} className="flex-1 py-4 bg-white border border-slate-300 hover:border-[#F26522] hover:text-[#F26522] text-slate-700 font-bold rounded-xl transition-colors flex items-center justify-center gap-2">
